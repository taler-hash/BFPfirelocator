...
5. after cloning you should see **BFP Fire Tracker** folder open that folder. To build the app run this command inside of the folder youve just open just run this one time run ( run this after finishing the other one)
  - `docker compose build --no-cache app && docker compose up app  && docker exec -d app bash "cd /var/www/html/app && chmod 777 -R ./ && cp .env.example .env && composer install && php artisan config:clear && php artisan migrate:fresh --seed && npm install"`
7. To start the app run this inside of the mentioned folder `docker compose up -d && docker exec -d app bashc "composer run dev"`
